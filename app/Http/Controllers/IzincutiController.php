...
class IzincutiController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Ambil input, fallback ke tanggal hari ini
        ...
        if ($jmlhari > $sisacuti) {
            return redirect('/presensi/izin')->with(['error' => 'Jumlah Hari Sudah Melebihi Batas Maksimal Jumlah Hari Cuti Dalam 1 Tahun, Sisa Cuti Anda ' . $sisacuti . ' Hari']);
        } else if ($cekpresensi->count() > 0) {
            $blacklistdate = "";
            foreach ($datapresensi as $d) {
                $blacklistdate .= date('d-m-Y', strtotime($d->tgl_presensi)) . ",";
            }
            return redirect('/presensi/izin')->with(['error' => 'Tidak Dapat Melakukan Pengajuan Izin Sakit, Anda Sudah Melakukan Presensi Pada Tanggal ' . $blacklistdate . ' Silahkan Ganti Tanggalnya !']);
        
        } else if ($cekpengajuan->count() > 0) {
            return redirect('/presensi/izin')->with(['error' => 'Tidak Dapat Melakukan Pengajuan Izin Sakit, Anda Sudah Melakukan Pengajuan Sebelumnya Pada Tanggal ' . $blacklistdate . ' Silahkan Ganti Tanggalnya !']);
        } else {
